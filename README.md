...
|---|---|---|
| 🟢 **DJ 6 AI host** ✨ new | PR [#67](https://github.com/ufoptg/MuSicX/pull/67) | In-app AI DJ that curates the next tracks (OpenRouter) and speaks sparse, natural banter with cloud TTS (Deepgram Flux via OpenRouter) or device TTS. Start from *Library Mix → DJ 6*, the player menu, or “DJ 6 play …”. Settings under *Integrations → DJ 6*. Optional **Talk to DJ 6** push-to-talk (must say **DJ 6** — e.g. “DJ 6 skip”) — no continuous mic listening. |
